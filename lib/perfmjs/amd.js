...
perfmjs.plugin('amd', function($$) {
    ...
    loader.loadParsedRequireStack = function(modules, needLoadModules, countdown, deferred) {
        ...
        for (var i = 0; i < needLoadModules.length; i++) {
            ...
            if (appConfig.option('amd.baseUrl') && appConfig.option('amd.modules') && appConfig.option('amd.modules')[thisNeedLoadModule]) {
                if (appConfig.option('amd.modules')[thisNeedLoadModule]['css'] && appConfig.option('amd.modules')[thisNeedLoadModule]['css'].length > 0) {
                    $$.utils.forEach(appConfig.option('amd.modules')[thisNeedLoadModule]['css'], function(item, index) {
                        needLoadResources[needLoadResources.length] = item;
                    });
                }
                if (appConfig.option('amd.modules')[thisNeedLoadModule]['js'] && appConfig.option('amd.modules')[thisNeedLoadModule]['js'].length > 0) {
                    $$.utils.forEach(appConfig.option('amd.modules')[thisNeedLoadModule]['js'], function(item, index) {
                        needLoadResources[needLoadResources.length] = item;
                    });
                }
    ...
    loader.parseRequireStack = function(modules, needLoadModules) {
        ...
        for (var x = 0; x < modules.length; x++) {
            ...
            //if (parsedRequireStack[thisModule]) { //fixed for a bug while pre-load adm js files in include.js
            if (defines[thisModule]) {
                continue;
            }
