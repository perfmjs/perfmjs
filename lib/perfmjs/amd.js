        ...
        for (var i = 0; i < needLoadModules.length; i++) {
            ...
            if (appConfig.option('amd.baseUrl') && appConfig.option('amd.modules') && appConfig.option('amd.modules')[thisNeedLoadModule]) {
                ...
                if ($$.utils.toBoolean(appConfig.option('amd.modules')[thisNeedLoadModule]['alias'])) {
                    //开始处理shim配置
                    if (appConfig.option('amd.modules')[thisNeedLoadModule]['shim']) {
                        var shimDeps = appConfig.option('amd.modules')[thisNeedLoadModule]['shim']['deps'];
                        var shimExports = appConfig.option('amd.modules')[thisNeedLoadModule]['shim']['exports'];
                        var shimInit = appConfig.option('amd.modules')[thisNeedLoadModule]['shim']['init'];
                        if (shimExports || shimInit) {
                            define(thisNeedLoadModule, shimDeps, shimInit || (shimExports && function() {return $$.utils.getGlobal(shimExports);}));
                        }
                    }
                    var shimAnonymousDefineId = appConfig.option('amd.modules')[thisNeedLoadModule]['anonymousDefineId'];
                    if (shimAnonymousDefineId && !anonymousDefineId[thisNeedLoadModule]) {
                        anonymousDefineId[shimAnonymousDefineId] = thisNeedLoadModule;
                    }
                    //结束处理shim配置
