System.register(['perfmjs/utils'], function(exports_1) {
    var utils_1;
    var BetBaseSuanfa, betBaseSuanfa;
    return {
        setters:[
            function (_utils_1) {
                utils_1 = _utils_1;
        ...
        execute: function() {
             ...
             * 投注基础算法
             */
            BetBaseSuanfa = (function () {
                function BetBaseSuanfa() {
                }
                /**
                 ...
                 */
                BetBaseSuanfa.prototype.rndNumber = function (mins, maxs, killingCodes) {
                    var codeNum = 1; //选1个号码
                    //防止杀掉了所有的号码
                 ...
                 */
                BetBaseSuanfa.prototype.rndNumbersAndFormat = function (mins, maxs, nums, isChongfu, isOrder, tmps, killingCodes, fillIntWith2Char) {
                    ...
                    if (tmps == undefined || tmps.length < 1)
                        tmps = "";
                    else
                        tmps = "," + tmps;
                    var tnum;
                 ...
                 * fillIntWith2Char: true-e.g.将'1,4,5,12'变成"01,04,05,12", 默认为false-e.g.将'01,04,5,12'变成"1,4,5,12"
                 */
                BetBaseSuanfa.prototype.formartBallNum = function (balls, fillIntWith2Char) {
                    var ballArray = [];
                    fillIntWith2Char = fillIntWith2Char || false;
                    if (fillIntWith2Char) {
                        utils_1.utils.forEach(balls.toString().split(','), function (item) {
                            ballArray[ballArray.length] = (utils_1.utils.toNumber(item) < 10 && item.length < 2) ? '0' + item : item;
                        });
                    }
                    else {
                        utils_1.utils.forEach(balls.toString().split(','), function (item) {
                            ballArray[ballArray.length] = utils_1.utils.toNumber(item) + "";
                        });
                    }
                    return ballArray.join(',');
                };
                //isOrder =-1 小到大，1大到小,0不排序
                BetBaseSuanfa.prototype.getSorts = function (mins, isOrder) {
                    var tmps = mins || '1';
                    if (Math.abs(isOrder) == 1)
                        tmps = isOrder == 1 ? tmps.split(",").sort(function (x, y) { return parseFloat(x) > parseFloat(y) ? -1 : 1; }).join(",") : tmps.split(",").sort(function (x, y) { return parseFloat(x) > parseFloat(y) ? 1 : -1; }).join(",");
                    return tmps;
                };
                /*FIXME, 计算选择的注数(该方法有待完善), e.g getCodeNumber(6,7,2) */
                BetBaseSuanfa.prototype.getCodeNumber = function (redNumOfOneZhu, redNum, blueNum) {
                    if (redNum < redNumOfOneZhu)
                        return 0;
                    if (blueNum == 0)
                        return 0;
                    if (redNum == redNumOfOneZhu)
                 ...
                 */
                BetBaseSuanfa.prototype.combinDanTuoArea = function (minSumOfDanPlusTuo, dan, tuo) {
                    if (dan <= 0 || dan >= minSumOfDanPlusTuo)
                        return 0;
                 ...
                 */
                BetBaseSuanfa.prototype.combination = function (m, n, iszhuhe) {
                    if (m < 0 || n < 0 || m < n)
                        return 0;
                ...
                return BetBaseSuanfa;
            })();
            exports_1("betBaseSuanfa", betBaseSuanfa = new BetBaseSuanfa());
        }
    }
});
