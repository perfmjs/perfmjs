...
declare module "angular2/angular2" {
  ...
  class AppView implements  ChangeDispatcher,  EventDispatcher {
     ...
     * `<li templates="for #item of items">`, where "player" and "item" are locals, respectively.
     */
    locals: Locals;
   ...
   * ```
   * 
   * We use this with the following HTML templates:
   * 
   * ```
   ...
   * `@View` annotation specifies the HTML templates to use, and lists the directives that are active
   * within the templates.
   *
   * When a component is instantiated, the templates is loaded into the component's shadow root, and
   * the expressions and statements in the templates are evaluated against the component.
   *
   ...
   * instance.
   *  4. It creates a shadow DOM on the selected component's host element and loads the templates into
   * it.
  ...
  /**
   * Cache that stores the AppProtoView of the templates of a component.
   ...
   * as well as in
   * templates with `*ng-for="of"` directive.
   * 
   ...
   * - loads the selected templates into the shadow DOM.
   * - creates a child {@link Injector} which is configured with the `appInjector` for the
   * {@link Component}.
   *
   * All templates expressions and statements are then evaluated against the component instance.
   *
   ...
   * 
   * When a templates is instantiated, it also must instantiate the corresponding directives in a
   ...
   * - `viewContainer: ViewContainerRef` to control child templates instantiation, for
   ...
   * with `@Optional()`.
   * This explicitly permits the author of a templates to treat some of the surrounding directives as
   * optional.
   * 
   * ```
   * @Directive({ selector: '[my-directive]' })
   ...
   * }
   * ```
   * In our HTML templates, we can then add this behavior to a `<div>` or any other element with the
   * `tooltip` selector,
   * like so:
   * 
   * ```
   ...
   * ```
   * 
   * Directives can also control the instantiation, destruction, and positioning of inline templates
   * elements:
   * 
   ...
   * runtime.
   * The <a href='/angular2/angular2/ViewContainerRef'><code>ViewContainerRef</code></a> is created as a result of `<templates>` element, and represents a
   ...
   * `<templates>` element. Thus a
   * directive in a child view cannot inject the directive that created it.
   * 
   * Since directives that create views via ViewContainers are common in Angular, and using the full
   * `<templates>` element syntax is wordy, Angular
   * also supports a shorthand notation: `<li *foo="bar">` and `<li templates="foo: bar">` are
   * equivalent.
   * 
   * Thus,
   * 
   ...
   * the directive
   * controller is correctly instantiated on the `<templates>` element rather than the `<li>` element.
   * 
   * 
   * ## Example
   * 
   * Let's suppose we want to implement the `unless` behavior, to conditionally include a templates.
   * 
   ...
   * ```
   * 
   * We can then use this `unless` selector in a templates:
   * ```
   * <ul>
   *   <li *unless="expr"></li>
   * </ul>
   * ```
   * 
   * Once the directive instantiates the child view, the shorthand notation for the templates expands
   * and the result is:
   * 
   * ```
   * <ul>
   *   <templates [unless]="exp">
   *     <li></li>
   *   </templates>
   *   <li></li>
   * </ul>
   * ```
   * 
   * Note also that although the `<li></li>` templates still exists inside the `<templates></templates>`,
   * the instantiated
   * view occurs on the second `<li></li>` which is a sibling to the `<templates>` element.
   * 
   * @exportedAs angular2/annotations
   */
  ...
  /**
   * The `NgFor` directive instantiates a templates once per item from an iterable. The context for
   * each instantiated templates inherits from the outer context with the given loop variable set
   * to the current item from the iterable.
   * 
   * It is possible to alias the `index` to a local variable that will be set to the current loop
   * iteration in the templates context.
   * 
   ...
   * - `<li templates="ng-for #item of items; #i = index">...</li>`
   * - `<templates [ng-for] #item [ng-for-of]="items" #i="index"><li>...</li></templates>`
   * 
   ...
   * - `<div templates="ng-if condition">...</div>`
   * - `<templates [ng-if]="condition"><div>...</div></templates>`
   * 
   * @exportedAs angular2/directives
   */
  class NgIf {
  ...
  /**
   * The `NgSwitch` directive is used to conditionally swap DOM structure on your templates based on a
   * scope expression.
   * Elements within `NgSwitch` but without `NgSwitchWhen` or `NgSwitchDefault` directives will be
   * preserved at the location as specified in the templates.
   * 
   ...
   *   <templates [ng-switch-when]="whenExpression1">...</templates>
   *   <templates [ng-switch-when]="whenExpression1">...</templates>
   *   <templates [ng-switch-default]>...</templates>
   * </ANY>
   * ```
   * 
   * @exportedAs angular2/directives
   ...
   * <templates [ng-switch-when]="contextVariable">...</templates>
   * 
   * // match against a constant string
   * <templates [ng-switch-when]="'stringValue'">...</templates>
   * ```
   * 
   * @exportedAs angular2/directives
   */
   ...
   * @View({
   *      directives: [formDirectives],
   *      templates: "<input type='text' [ng-form-control]='loginControl'>"
   *      })
   * class LoginComp {
   ...
   * @View({
   *      directives: [formDirectives],
   *      templates: "<form [ng-form-model]='loginForm'>" +
   *              "Login <input type='text' ng-control='login'>" +
   *              "Password <input type='password' ng-control='password'>" +
   *              "<button (click)="onLogin()">Login</button>" +
   *              "</form>"
   ...
   * - we need the parse at least for the `templates` attribute to match
   *   directives in it
