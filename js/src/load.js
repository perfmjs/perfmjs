    ...
    if (isAsync) {
        api.load = function () {
            ...
            each(args, function (item, i) {
            	if (perfmjs.utils.isArray(item)) {
            		//for perfmjs: 处理第一个参数是资源文件地址数组的情况
            		var item_els = item;
	        		var item_len = item.length;
	        		for (var i = 0; i < item_len; i++) {
	        			item             = getAsset(item_els[i]);
        ...
        api.load = function () {
            ...
            if (!!next) {
            	...
            	if (perfmjs.utils.isArray(args[0])) {
                	//for perfmjs: 处理第一个参数是资源文件地址数组的情况
            		var item_els = args[0];
            		var item_len = item_els.length;
            		for (var i = 0; i < item_len; i++) {
            			load(getAsset(item_els[i]));
            		}
            	} else {
	                // single item
